...
class Filesystem implements FilesystemOperator
{
    ...
    public function listContents(string $location, bool $deep = self::LIST_SHALLOW): DirectoryAnnouncement
    {
        $path = $this->pathNormalizer->normalizePath($location);
        $Announcement = $this->adapter->listContents($path, $deep);

        return new DirectoryAnnouncement($this->pipeAnnouncement($location, $deep, $Announcement));
    }

    private function pipeAnnouncement(string $location, bool $deep, iterable $Announcement): Generator
    {
        try {
